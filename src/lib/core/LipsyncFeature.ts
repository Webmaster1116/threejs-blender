...
import {
  ...
  TextToSpeechFeatureDependentInterfaceMixin,
} from "./awspack/TextToSpeechFeatureDependentInterface";
 ...
 */
export class LipsyncFeature extends ManagedAnimationLayerInterfaceMixin(
  TextToSpeechFeatureDependentInterfaceMixin(AbstractHostFeature)
) {
  static featureName = "LipsyncFeature";
  // implements
  //   TextToSpeechFeatureDependentInterface,
  //   ManagedAnimationLayerInterface
  /**
  ...
  constructor(
    ...
    visemeLayers.forEach(
      ({
        ...
        name: string;
        animation: any;
        visemeMap: typeof DefaultVisemeMap;
      }): void => {
    ...
    talkingLayers.forEach(
      ({ name, animation }: { name: string; animation: any }) => {
  ...
  _onPlay() {
    ...
    this.enable(0, (x: number) => x);

    // Set the current animations
    [this._visemeLayers, this._talkingLayers].forEach((layers) => {
      Object.entries(layers).forEach(([layerName, animationName]: any) => {
        if (this._managedLayers[layerName].animations[animationName].isActive) {
          this._host?.AnimationFeature.resumeAnimation(
            layerName,
            animationName
          );
        }
  ...
  _onStop() {
    ...
    this.disable(0, (x: number) => x);
    [
      // Pause the current animations
      (this._visemeLayers, this._talkingLayers),
    ].forEach((layers) => {
      Object.entries(layers).forEach(([layerName, animationName]: any) => {
        if (this._managedLayers[layerName].animations[animationName].isActive) {
   ...
   */
  async _onViseme({ mark }: any) {
    Object.entries(this._visemeLayers).forEach(([layerName, animName]: any) => {
  ...
  async _animateHeldViseme(
    ...
    // console.log('startPromise.promise', startPromise.promise);
    ...
    if (!startPromise.canceled) {
      ...
      // console.log('holdPromise.promise', holdPromise.promise);
      await holdPromise.promise;
