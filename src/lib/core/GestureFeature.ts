...
{
  static featureName = "GestureFeature";
  holdTime: number;
  ...
  constructor(
    host: HostObject,
    { holdTime = 3, minimumInterval = 1, layers = {} as any } = {}
  ) {
    super(host);
    ...
    Object.entries(layers).forEach(([name, options = {}]) => {
      this.registerLayer(name, options as any);
    });
  }
   ...
   * @returns {number}
   */
  private _getMinimumInterval(layer: any, animation: any): number {
  ...
  registerLayer(name: string, options = {}) {
    ...
    if (this._managedLayers[name].isActive) {
      this._host?.AnimationFeature.getAnimations(name).forEach((anim: any) => {
  ...
  registerAnimation(
    ...
    if (!(options.words instanceof Array) && !(options.words instanceof Set)) {
      options.words =
        DefaultGestureWords[
          animationName as keyof typeof DefaultGestureWords
        ] || [];
    }
   ...
   */
  createGestureMap() {
    const ctor = this.constructor as typeof GestureFeature;
    const gestureMap: any = {};

    Object.entries(this._managedLayers).forEach(
   ...
   */
  createGenericGestureArray(layers: string[]): any[] {
    const ctor = this.constructor as typeof GestureFeature;
    const genericGestures: any[] = [];
  ...
  playGesture(
    layerName: string,
    animationName: string,
    { holdTime, minimumInterval, transitionTime, force = false }: any = {}
  ) {
