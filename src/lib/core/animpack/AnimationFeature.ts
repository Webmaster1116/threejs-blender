...
export class AnimationFeature extends AbstractHostFeature {
  static featureName = "AnimationFeature";
  static EVENTS = {
   ...
   * @returns {number=}
   */
  _validateIndex(index: number, existing = true) {
  ...
  _createSingleState(
    options: {
      ...
      timeScale: 1,
      loopCount: Infinity,
      blendMode: LayerBlendModes[DefaultLayerBlendMode],
    }
  ): SingleState {
  ...
  _createQueueState(
    ...
    return new QueueState(options, queueStates as any);
  }
  ...
  _createBlend1dState(
    ...
    const blendStates: SingleState[] = [];
    blendStateOptions.forEach(
      (
        blendOptions:
          | {
        ...
        return blendStates.push(
          this._createSingleState({
            ...blendOptions,
            blendMode:
              options.blendMode || LayerBlendModes[DefaultLayerBlendMode],
          })
        );
      }
    );
  ...
  _createBlend2dState(
    ...
    const blendStates: SingleState[] | undefined = [];
    blendStateOptions.forEach(
      (
        blendOptions:
          | {
   ...
   */
  _createRandomAnimationState(options: any) {
    const { subStateOptions = [] } = options;

    const subStates: SingleState[] = [];
    subStateOptions.forEach(
      (
        subStateOptions:
          | {
              name?: string | undefined;
   ...
   */
  _validateNewAnimation(layerName: string | number, animationName: string) {
   ...
   */
  addLayer(name = "NewLayer", options = {}, index: number | undefined) {
    const numLayers = this._layers.length;
    ...
    const ctor = this.constructor as typeof AnimationFeature;
    this.emit(ctor.EVENTS.addLayer, eventData);
  ...
  removeLayer(name: string | number) {
    ...
    const ctor = this.constructor as typeof AnimationFeature;
    this.emit(ctor.EVENTS.removeLayer, { name, index });
   ...
   */
  moveLayer(name: string | number, index: number) {
   ...
   */
  renameLayer(currentName: string | number, newName: string) {
    // Make sure the name is valid
    ...
    const ctor = this.constructor as typeof AnimationFeature;
    this.emit(ctor.EVENTS.renameLayer, {
      oldName: currentName,
      newName: name,
    });
   ...
   * @returns {number}
   */
  getLayerWeight(name: string | number) {
    // Make sure the name is valid
    const layer = this._layerMap[name];
   ...
   */
  getAnimationBlendNames(layerName: string | number, animationName: any) {
    const layer = this._layerMap[layerName];
   ...
   */
  pauseLayerWeight(name: string | number) {
    // Make sure the name is valid
    const layer = this._layerMap[name];
   ...
   */
  resumeLayerWeight(name: string | number) {
    // Make sure the name is valid
    const layer = this._layerMap[name];
   ...
   */
  resumeLayer(name: string | number) {
    // Make sure the name is valid
    const layer = this._layerMap[name];
   ...
   * @returns {boolean}
   */
  getTransitioning(layerName: string | number) {
    // Make sure the layerName is valid
    const layer = this._layerMap[layerName];
   ...
   */
  getAnimations(layerName: string | number) {
    // Make sure the layerName is valid
    const layer = this._layerMap[layerName];
   ...
   * @returns {(string|null)}
   */
  getCurrentAnimation(layerName: string | number) {
    // Make sure the layerName is valid
    const layer = this._layerMap[layerName];
   ...
   * @returns {boolean}
   */
  getPaused(layerName: string | number) {
    // Make sure the layerName is valid
    const layer = this._layerMap[layerName];
   ...
   * @returns {string}
   */
  getAnimationType(layerName: string | number, animationName: any) {
    // Make sure the layerName is valid
    const layer = this._layerMap[layerName];
    ...
    return Object.keys(AnimationTypes).find(
      (typeName) =>
        AnimationTypes[typeName as keyof typeof AnimationTypes] === constructor
    );
  }
  ...
  addAnimation(
    ...
    const state =
      this[
        `_create${animationType.stateName}` as unknown as keyof AnimationFeature
      ](options);

    const name = layer.addState(state);
    const ctor = this.constructor as typeof AnimationFeature;
    // Notify that an animation has been added to the feature
    this.emit(ctor.EVENTS.addAnimation, { layerName, animationName: name });
    return name;
  }
   ...
   * @returns {boolean}
   */
  removeAnimation(layerName: string | number, animationName: any) {
    // Make sure the name is valid
    const layer = this._layerMap[layerName];
    ...
    const ctor = this.constructor as typeof AnimationFeature;
    if (removed === true) {
      this.emit(ctor.EVENTS.removeAnimation, { layerName, animationName });
    }
  ...
  renameAnimation(
    layerName: string | number,
    currentAnimationName: any,
    newAnimationName: any
  ) {
    const layer = this._layerMap[layerName];
    ...
    const ctor = this.constructor as typeof AnimationFeature;
    this.emit(ctor.EVENTS.renameAnimation, {
      layerName,
  ...
  playAnimation(
    ...
    const ctor = this.constructor as typeof AnimationFeature;
    this.emit(ctor.EVENTS.playAnimation, { layerName, animationName });
    ...
    return layer.playAnimation(
      ...
      () => {
        ...
        this.emit(ctor.EVENTS.stopAnimation, { layerName, animationName });
      },
      undefined,
      () => {
        // Notify that an animation has been interrupted
        this.emit(ctor.EVENTS.interruptAnimation, { layerName, animationName });
      },
      ({ name, canAdvance, isQueueEnd }: any) => {
        if (layer.currentAnimation === animationName) {
          // Notify that a new animation has begun
          this.emit(ctor.EVENTS.playNextAnimation, {
            layerName,
            animationName,
  ...
  playNextAnimation(
    ...
    const onNext = ({ name, canAdvance, isQueueEnd }: any) => {
      const ctor = this.constructor as typeof AnimationFeature;
      if (layer.currentAnimation === animationName) {
        // Notify that a new animation has begun
        this.emit(ctor.EVENTS.playNextAnimation, {
   ...
   */
  pauseAnimation(name: string | number) {
    const ctor = this.constructor as typeof AnimationFeature;
    const layer = this._layerMap[name];
    ...
    if (paused) {
      ...
      this.emit(ctor.EVENTS.pauseAnimation, {
        layerName: name,
        animationName,
      });
    }
  ...
  resumeAnimation(
    ...
    this.emit(ctor.EVENTS.resumeAnimation, {
      layerName,
      animationName,
    });
    ...
    return layer.resumeAnimation(
      ...
      () => {
        // Notify that an animation has stopped
        this.emit(ctor.EVENTS.stopAnimation, {
          layerName,
          animationName,
        });
      },
      ...
      () => {
        // Notify that an animation has been interrupted
        this.emit(ctor.EVENTS.interruptAnimation, {
          layerName,
          animationName,
        });
      },
   ...
   */
  stopAnimation(name: string | number) {
    const layer = this._layerMap[name];
