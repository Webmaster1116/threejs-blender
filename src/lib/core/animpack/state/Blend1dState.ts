 ...
 */
export class Blend1dState
  extends AbstractBlendState
  implements StateContainerInterface, AbstractState
{
  static stateName = "Blend1dState";

  declare _blendValue: number;
  ...
  addState(state: AbstractState, thresholdValue = 0, phaseMatch = false) {
   ...
   */
  setBlendThreshold(name: string, value: number) {
    // Make sure there is not already a state with this threshold
    const sameValue = this._thresholds.find(
  ...
  updateInternalWeight(factor: number) {
    ...
    if (this._phaseLeadState) {
      this._states.forEach((state) => {
        if (state.weight !== 0) {
          (state as SingleState).normalizedTime =
