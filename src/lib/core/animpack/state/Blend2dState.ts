...
export class Blend2dState extends AbstractBlendState {
  static stateName = "Blend2dState";
  declare _blendValueX: number;
  declare _blendValueY: number;
  declare _thresholds: { value?: number; name: string; phaseMatch: boolean }[];
  declare _triangles: number[][];
  /**
   * @constructor
   *
  ...
  updateInternalWeight(factor: number) {
    ...
    if (this._phaseLeadState) {
      ([...this._states.values()] as any as Blend2dState[]).forEach(
        (state, index) => {
          if (state.weight !== 0 && this._thresholds[index].phaseMatch) {
            (state as any as SingleState).normalizedTime =
              this._phaseLeadState.normalizedTime;
          }
        }
      );
    }
  ...
  _updateBlendWeights() {
    ...
    const p: [number, number] = [this._blendValueX, this._blendValueY];
   ...
   * @private
   */
  _setInfluenceTriangle(triangle: number[], p: [number, number]) {
    ...
    this._setPhaseLeadState([stateA, stateB, stateC] as AbstractBlendState[], [
      thresholdA.phaseMatch,
      thresholdB.phaseMatch,
      thresholdC.phaseMatch,
    ]);
  }
   ...
   * @private
   */
  _setInfluenceClosestPointInTriangles(p: [number, number]) {
    let globalClosestPoint: [number, number] | undefined;
    ...
    this._triangles.forEach((triangle, index) => {
      ...
      if (localMinDist < globalMinDist) {
        ...
        globalClosestPoint = [...localClosestPoint] as [number, number];
        closestTriangle = index;
      }
    });
    if (globalClosestPoint) {
   ...
   * @private
   */
  _setInfluenceClosestPointOnLine(p: [number, number]) {
    const closestPoint = MathUtils.closestPointOnLine(
      this._vertices[p[0]], //ankit
      this._vertices[p[1]],
      p
    );
    ...
    this._setPhaseLeadState(
      [
        stateA as any as AbstractBlendState,
        stateB as any as AbstractBlendState,
      ],
