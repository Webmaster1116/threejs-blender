...
export class MathUtils {
   ...
   */
  static closestPointOnLine(a: number[], b: number[], p: number[]) {
   ...
   * @returns {number}
   */
  static distanceSquared(a: number[], b: number[]) {
   ...
   */
  static getDelaunayTriangulation(vertices: any[]) {
    ...
    vertsWithSuper.forEach((newVert, newIndex) => {
      const invalidTriangles: { indices: any; edges: any[][] }[] = [];
      ...
      const boundingPoly: any[] = [];
      invalidTriangles.forEach((triangle) => {
        triangle.edges.forEach((edge: any[]) => {
          let count = 0;
          invalidTriangles.forEach((otherTriangle) => {
            if (triangle !== otherTriangle) {
              otherTriangle.edges.forEach((otherEdge: any[]) => {
                if (
    ...
    const trianglesToRemove: { indices: any; edges: any[][] }[] = [];
    triangles.forEach((triangle) => {
      triangle.indices.forEach((index: any) => {
   ...
   * @returns {boolean}
   */
  static isPointInTriangle(a: number[], b: number[], c: number[], p: any[]) {
   ...
   * @returns {number}
   */
  static triangleArea(a: number[], b: number[], c: number[]) {
    return Math.abs(
   ...
   */
  static cartesianToSpherical(x: number, y: number, z: number): number[] {
