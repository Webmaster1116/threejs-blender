...
export class HostObject extends Messenger {
  ...
  private _waits: Deferred[];
  protected _lastUpdate: number;

  AnimationFeature: any;
  GestureFeature: any;
  LipsyncFeature: any;
  PointOfInterestFeature: any;
  TextToSpeechFeature: any;
  ...
  update() {
    ...
    const ctor = this.constructor as typeof HostObject;
    // Notify listeners an update occurred
    this.emit(ctor.EVENTS.update, dt);
  ...
  wait(
    ...
    wait.promise.then(onComplete, onComplete);
    return wait;
  }
   ...
   */
  addFeature(
    featureClass: AbstractHostFeature,
    force = false,
    ...args: any[]
  ): boolean {
    const ctor = this.constructor as typeof HostObject;
    ...
    // Make sure the feature is a host feature
      ...
      !((featureClass as any).prototype instanceof AbstractHostFeature)
    ) {
      throw new Error(
        `Cannot add feature ${(featureClass as any).featureName} to host ${
          this.id
        }. FeatureClass must extend AbstractHostFeature.`
      );
    }

    // Check if the FeatureClass already exists on this object
    if (this._features[(featureClass as any).featureName] !== undefined) {
      if (force) {
        console.warn(
          `Feature ${
            (featureClass as any).featureName
          } already exists on host ${
            this.id
          }. Existing feature will be overwritten.`
        );
      } else {
        throw new Error(
          `Feature ${
            ...
            this.id
          }. Use 'force' argument to overwrite the feature.`
        );
      }
    }

    // Initialize the feature
    const feature = new (featureClass as any)(this, ...args);
    feature.installApi();

    this._features[(featureClass as any).featureName] = feature;
    this.emit(ctor.EVENTS.addFeature, (featureClass as any).featureName);
  ...
  removeFeature(typeName: string): boolean {
    ...
    if (this._features[typeName] === undefined) {
      ...
      // Remove the feature
      this.emit(ctor.EVENTS.removeFeature, typeName);
