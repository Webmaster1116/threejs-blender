...
export class TextToSpeechUtils {
  ...
  static autoGenerateSSMLMarks(
    ...
    const chunks = [];
    let index = 0;
    let ssmlMarkResult: any;
    let duplicateMarkToCheck: any[] = [];
   ...
   * @returns {string}
   */
  static addMarksToUnmarkedSentences(text: string, marks: string[]) {
   ...
   */
  static _getSentenceEnds(text: string) {
    ...
    while (result !== null) {
      ...
      const punctResult: any = endSentenceRegex.exec(result[0]);
      sentenceEnds.push(
  ...
  static _insertMarks(
    ...
    const markedWords = text.match(wordRegex)?.map((word) => {
      ...
      if (map.has(lowerCaseWord)) {
        const marks = map.get(lowerCaseWord)?.map((mark: string) => {
          if (checkDuplicate) {
   ...
   */
  static _processInputMap(map: any): Map<string, any[]> {
